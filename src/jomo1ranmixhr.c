...
SEXP jomo1ranmixhr(SEXP Y, SEXP Yimp, SEXP Yimp2, SEXP Yimpcat, SEXP X, SEXP Z, SEXP clus, SEXP beta, SEXP u, SEXP betapost, SEXP upost, SEXP omega,SEXP omegapost, SEXP covu, SEXP covupost, SEXP nstep, SEXP Sp, SEXP Sup, SEXP Y_numcat, SEXP num_con, SEXP a_start, SEXP flagrng){
...
double *cumclus, *allinvomega,gamma,eta,dx,u_new,precision, *invgamma, *invA, *Gammapr, *Gammastar,u_m,con,con2,deriv2,u_prop, lambda;
...
for (i=0;i<ncat;i++) {
	for (j=0;j<(INTEGER(Y_numcat)[i]-1);j++) {
		for (k=0;k<(INTEGER(Y_numcat)[i]-1);k++) {
			...
			else for (t=0;t<nj;t++) REAL(omega)[(pos+j+t*JY)+JY*nj*(pos+k)]=0;
...
for (i=0;i<ns;i++) {
		...
		for (t=0;t<IY;t++) {
			if (!ISNAN(REAL(Y)[t+(ncon+j)*IY])) {
				...
				for (k=0;k<pos;k++) {
					...
					for (kk=0;kk<pos;kk++) help4[k+pos*kk]=REAL(omega)[INTEGER(clus)[t]*JY+k+nj*JY*kk];
					for (kk=0;kk<(INTEGER(Y_numcat)[j]-1);kk++) help5[k+pos*kk]=REAL(omega)[INTEGER(clus)[t]*JY+k+nj*JY*(pos+kk)];
				}
				r8mat_pofac(pos,help4, help6,3);
				...
				r8mat_pofac((INTEGER(Y_numcat)[j]-1),omegadrawmiss, omegamm,4);
				flag=0;
				kk=0;
	...
	wishart_sample(JY*JZ,nj-1,invomega3,newomega2, help5,sumzi,incrzz,mu3,fl);
	...
	//					if (k==j) invA[k+JY*j]=1;
	//					else invA[k+JY*j]=0;
	//				}
	//			}
	//		}
	u_m=newton_raphson(u_new,precision, dx,eta,JY,nj,allinvomega,invomega,invA,help,help2,gamma,Gammastar);
	if (u_m!=-9999) {
		con=-log_f_u(eta, u_m, JY, nj, allinvomega, invomega,  invA,  help,  help2, gamma, Gammastar);
		deriv2=derive2_f_u(dx,eta, u_m, JY, nj, allinvomega, invomega,  invA,  help,  help2, gamma, Gammastar,con);	
	...
	if (u_m==-9999) {
		u_m=a;
		con=-log_f_u(eta, u_m, JY, nj, allinvomega, invomega,  invA,  help,  help2, gamma, Gammastar);
		deriv2=derive2_log_f_u(dx,eta, u_m, JY, nj, allinvomega, invomega,  invA,  help,  help2, gamma, Gammastar);
		lambda=sqrt(-5/(4*deriv2));
		u_prop=lambda*t_sample(4,fl)+u_m;
		con2=exp(log_f_u(eta, u_prop, JY, nj, allinvomega, invomega, invA,help,help2,gamma, Gammastar)-log_f_u(eta, u_new, JY, nj, allinvomega, invomega, invA,help,help2, gamma,Gammastar))*h_u(u_new,u_m,lambda)/h_u(u_prop,u_m,lambda);
		if ((( double ) unif_rand ( ) / ( double ) RAND_MAX)<r8_min(1,con2)) u_new=u_prop;
	...
	u_new=a;	
	for (c=0;c<nj;c++) {	
		for (j=0;j<JY*JY;j++) newomega2[j]=0;
		...
		while ((flag==0)&(kk<100)) {	
			...
			r8mat_pofac(JY,newomega2,help,14);
			r8mat_poinv(JY, help,invomega2);
			for (jj=1;jj<JY;jj++) for (tt=0;tt<jj;tt++) invomega2[jj+JY*tt]=invomega2[tt+JY*jj];
			for (k=0;k<JY;k++) {
				for (j=0;j<JY;j++) {
					if (fixomega[k+JY*j]==1) {
		...
		if (flag!=0) {	
			for(k=0;k<JY;k++)  for(j=0;j<JY;j++)  REAL(omega)[(c*JY+j)+k*(JY*nj)]=invomega2[j+JY*k];
		}
	}
