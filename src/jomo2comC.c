...
int Iu, Ju, IZ, JZ, nj,c,fl, currncat, IY2, JY2, IX2,JX2,Ib2, Jb2, ncon2, ncat2, JYm, JXm, MCMC, np, np2, *mpid2red;
...
double maxx,maxim,maxim2, minim,*sumxy, *sumxi, *uj, *xi, *ziu, *incrxx, *incrxy, *newu, *mu3,  *mu4, *help7, *help8, *help9, *invomega4, *newomega2,*missing2, *Y2red, *X2red;
double *covu1, *covu2, *covu12, *fixomega2, *Y2impred, *listh8, *listomega;
					...
					if (!ISNAN(Y2red[t+(ncon2+j)*Iu])) {
						...
						if (Y2red[t+(ncon2+j)*Iu]==INTEGER(Y2_numcat)[j]) {
							...
							while ((flag==0)&(kk<10000)) {
								...
								minim=minvec((INTEGER(Y2_numcat)[j]-1),newbeta);
								if ((minim>-3)&(maxim<4)) {
									maxim2=argmaxvec((INTEGER(Y2_numcat)[j]-1),newbeta);
									if (((maxim2+1)==Y2red[t+(ncon2+j)*Iu])&(maxim>0)) {
										for (k=0;k<currncat;k++) {
											imp2[t+(k+pos)*Iu]=newbeta[k];
											REAL(u)[t+(JY*JZ+k+pos)*Iu]=newbeta[k]-betaX[k];
										}
