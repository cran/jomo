jomo.glm <-
  function(formula, data, beta.start=NULL, l1cov.start=NULL, l1cov.prior=NULL, nburn=1000, nbetween=1000, nimp=5, output=1, out.iter=10, family="binomial") {
    ...
    if (family=="gaussian") {
      jomo.lm(formula, data, beta.start=beta.start, l1cov.start=l1cov.start, l1cov.prior=l1cov.prior, nburn=nburn, nbetween=nbetween, nimp=nimp, output=output, out.iter=out.iter)
    }
    if (family=="binomial") {
      ...
      Ysubcat<-c(Ysub)
      .Call("jomoglm", Ysub, Ysubimp, Ysubcat, submod, order.sub, Y, Yimp, Yimp2, Y.cat, X, betaY.start,bYpost, betait,bpost, varY.start,vYpost,covit,opost, nburn, varY.prior, l1cov.prior,Y.numcat,ncolYcon,out.iter, PACKAGE = "jomo")
      #betapost[,,1]=bpost
      #upostall[,,1]=upost
      #omegapost[,,(1)]=opost
      #covupost[,,(1)]=cpost
      ...
      for (i in 2:nimp) {
        ...
        .Call("jomoglm", Ysub, Ysubimp, Ysubcat, submod, order.sub, Y, Yimp, Yimp2, Y.cat, X, betaY.start,bYpost,betait,bpost, varY.start,vYpost,covit,opost, nbetween, varY.prior, l1cov.prior,Y.numcat,ncolYcon,out.iter, PACKAGE="jomo")
        betapost[,,(i-1)]=bpost
