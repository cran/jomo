jomo.clmm <-
  function(formula, data, level=rep(1,ncol(data)), beta.start=NULL, l2.beta.start=NULL, u.start=NULL, l1cov.start=NULL, l2cov.start=NULL, l1cov.prior=NULL, l2cov.prior=NULL, a.start=NULL, a.prior=NULL, nburn=1000, nbetween=1000, nimp=5, meth="common", output=1, out.iter=10) {
    ...
    if (nimp<2) {
      nimp=2
      cat("Minimum number of imputations:2. For single imputation using function jomo.clmm.MCMCchain\n")
    }
    ...
    fit.cr<-ordinal::clmm(formula,data=data, na.action = na.omit, Hess=T, link = "probit")
    colnamysub<-all.vars(formula[[2]])
    Ysub<-get(colnamysub,pos=data)
    stopifnot(is.factor(Ysub))
    Ysub.ncat<-nlevels(Ysub)
    betaY.start<-c(coef(summary(fit.cr))[-c(1:Ysub.ncat-1),1],coef(summary(fit.cr))[c(1:Ysub.ncat-1),1])
    varY.start<-1
    varY.prior<-1
    ...
    uY.start<-matrix(0,nlevels(clus),ncol(ordinal::VarCorr(fit.cr)[[1]]))
    ...
    diagVar<-as.data.frame(ordinal::VarCorr(fit.cr))[1:ncol(ordinal::VarCorr(fit.cr)[[1]]),4]
    covuY.start<-ordinal::VarCorr(fit.cr)[[1]][1:ncol(ordinal::VarCorr(fit.cr)[[1]]),1:ncol(ordinal::VarCorr(fit.cr)[[1]])]
    covuY.prior<-ordinal::VarCorr(fit.cr)[[1]][1:ncol(ordinal::VarCorr(fit.cr)[[1]]),1:ncol(ordinal::VarCorr(fit.cr)[[1]])]
    if (kappa(covuY.start)>10^8) {
      covuY.prior<-diag(1, ncol(ordinal::VarCorr(fit.cr)[[1]]))
      covuY.start<-diag(1, ncol(ordinal::VarCorr(fit.cr)[[1]]))
    }
    ncolYcon<-rep(NA,4)
    ncolY2con<-rep(NA,4)
    ...
    if (!is.null(Y2)) {
      if (meth=="common") {
        .Call("jomo2smcC", Ysub, Ysubimp, Ysubcat, submod, order.sub, submod.ran, Y, Yimp, Yimp2, Y.cat.tot, Y2, Y2imp, Y2imp2, Y2.cat.tot, X, X2, Z, clus,betaY.start,bYpost, betait, beta2it, uit,uY.start,bpost, upost, uYpost, b2post, varY.start, vYpost, covit,opost, covuY.start, cuYpost, covuit, cpost, nburn, varY.prior, covuY.prior, l1cov.prior,l2cov.prior,Y.numcat.tot, Y2.numcat.tot, Ysub.ncat, ncolYcon,ncolY2con, out.iter, 0, 2, PACKAGE = "jomo")
      } else {
        .Call("jomo2hrsmcC", Ysub, Ysubimp, Ysubcat, submod, order.sub, submod.ran, Y, Yimp, Yimp2, Y.cat.tot, Y2, Y2imp, Y2imp2, Y2.cat.tot, X, X2, Z, clus,betaY.start,bYpost, betait, beta2it, uit,uY.start,bpost, upost, uYpost, b2post, varY.start, vYpost, covit,opost, covuY.start, cuYpost, covuit, cpost, nburn, varY.prior, covuY.prior, l1cov.prior,l2cov.prior,Y.numcat.tot, Y2.numcat.tot, Ysub.ncat, ncolYcon,ncolY2con, a.start, a.prior, out.iter, 0, 2, PACKAGE = "jomo")
      }
    } else {
      if (meth=="common") {
        .Call("jomo1ransmcC", Ysub, Ysubimp, Ysubcat, submod, order.sub, submod.ran, Y, Yimp, Yimp2, Y.cat.tot, X, Z, clus,betaY.start,bYpost, betait,uit,uY.start,bpost, upost, uYpost, varY.start, vYpost, covit,opost, covuY.start, cuYpost, covuit, cpost, nburn, varY.prior, covuY.prior, l1cov.prior,l2cov.prior,Y.numcat.tot, Ysub.ncat, ncolYcon,out.iter, 0, 2, PACKAGE = "jomo")
      } else {
        .Call("jomo1ranhrsmcC", Ysub, Ysubimp, Ysubcat, submod, order.sub, submod.ran, Y, Yimp, Yimp2, Y.cat.tot, X, Z, clus,betaY.start,bYpost, betait,uit,uY.start,bpost, upost, uYpost, varY.start, vYpost, covit,opost, covuY.start, cuYpost, covuit, cpost, nburn, varY.prior, covuY.prior, l1cov.prior,l2cov.prior,Y.numcat.tot, Ysub.ncat, ncolYcon, a.start, a.prior, out.iter, 0, 2, PACKAGE = "jomo")
      }
    }
    #betapost[,,1]=bpost
    #upostall[,,1]=upost
    ...
    for (i in 2:nimp) {
      ...
      if (!is.null(Y2)) {
        ...
        if (meth=="common") {
          .Call("jomo1ransmcC", Ysub, Ysubimp, Ysubcat, submod, order.sub, submod.ran, Y, Yimp, Yimp2, Y.cat.tot, X, Z, clus,betaY.start,bYpost, betait,uit,uY.start,bpost, upost, uYpost, varY.start, vYpost, covit,opost, covuY.start, cuYpost, covuit, cpost, nbetween, varY.prior, covuY.prior, l1cov.prior,l2cov.prior,Y.numcat.tot, Ysub.ncat, ncolYcon, out.iter, 0, 2, PACKAGE = "jomo")
        } else {
          .Call("jomo1ranhrsmcC", Ysub, Ysubimp, Ysubcat, submod, order.sub, submod.ran, Y, Yimp, Yimp2, Y.cat.tot, X, Z, clus,betaY.start,bYpost, betait,uit,uY.start,bpost, upost, uYpost, varY.start, vYpost, covit,opost, covuY.start, cuYpost, covuit, cpost, nbetween, varY.prior, covuY.prior, l1cov.prior,l2cov.prior,Y.numcat.tot, Ysub.ncat, ncolYcon, a.start, a.prior, out.iter, 0, 2, PACKAGE = "jomo")
        }
      }
      betapost[,,(i-1)]=bpost
    ...
    colnames(betaYpostmean)<-names(fit.cr$coefficients)[c(Ysub.ncat:length(fit.cr$coefficients),1:(Ysub.ncat-1))]
    rownames(betaYpostmean)<-colnamysub
    colnames(covuYpostmean)<-rownames(covuYpostmean)<-colnames(uYpostmean)<-rownames(fit.cr$ST[[1]])
    ...
    if (isnullcat2aux==0) {
      for (i in 1:ncol(Y2.aux.cat)) {
        imp[,(1+ncol(Y)+ncolY2con[3]+i)]<-as.factor(imp[,(1+ncol(Y)+ncolY2con[3]+i)]) 
        levels(imp[,(1+ncol(Y)+ncolY2con[3]+i)])<-previous_levels2aux[[i]]
      }
    }
