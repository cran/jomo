jomo1ranmixhr <-
function(Y_con, Y_cat, Y_numcat, X=matrix(1,nrow(Y_cat),1), Z=matrix(1,nrow(Y_cat),1), clus, betap=matrix(0,ncol(X),(ncol(Y_con)+(sum(Y_numcat)-length(Y_numcat)))), up=matrix(0,nrow(unique(clus)),ncol(Z)*(ncol(Y_con)+(sum(Y_numcat)-length(Y_numcat)))), covp=matrix(diag(1,ncol(betap)),ncol(betap)*nrow(unique(clus)),ncol(betap),2), covu=diag(1,ncol(up)), Sp=diag(1,ncol(betap)), Sup=diag(1,ncol(up)), nburn=100, nbetween=100, nimp=5,a=ncol(betap),meth="random", output=1, out.iter=10) {
  ...
  if (meth=="fixed") {
    .Call("jomo1ranmixhf", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betait,uit,bpost,upost,covit,opost, covuit,cpost,nburn, Sp,Sup,Y_numcat, ncol(Y_con),ait,out.iter, PACKAGE = "jomo")
  }
  if (meth=="random") {
    .Call("jomo1ranmixhr", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betait,uit,bpost,upost,covit,opost, covuit,cpost,nburn, Sp,Sup,Y_numcat, ncol(Y_con),ait,out.iter, PACKAGE = "jomo")
  }
  #betapost[,,1]=bpost
  #upostall[,,1]=upost
  #omegapost[,,(1)]=opost
  ...
  for (i in 2:nimp) {
    ...
    if (meth=="fixed") {
      .Call("jomo1ranmixhf", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betait,uit,bpost,upost,covit,opost, covuit,cpost,nbetween, Sp,Sup,Y_numcat, ncol(Y_con),ait,out.iter, PACKAGE = "jomo")
    }
    if (meth=="random") {
      .Call("jomo1ranmixhr", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betait,uit,bpost,upost,covit,opost, covuit,cpost,nbetween, Sp,Sup,Y_numcat, ncol(Y_con),ait,out.iter, PACKAGE = "jomo")
    }
    betapost[,,(i-1)]=bpost
    upostall[,,(i-1)]=upost
    omegapost[,,(i-1)]=opost
