jomo.coxph <-
  function(formula, data,  beta.start=NULL, l1cov.start=NULL, l1cov.prior=NULL, nburn=1000, nbetween=1000, nimp=5, output=1, out.iter=10) {
    ...
    .Call("jomocox", Ysub, submod, order.sub, Y, Yimp, Yimp2, Y.cat, X, betaY.start,bYpost, betait,bpost, covit,opost, nburn, l1cov.prior,Y.numcat,ncolYcon,out.iter, PACKAGE="jomo")
    #betapost[,,1]=bpost
    #upostall[,,1]=upost
    #omegapost[,,(1)]=opost
    #covupost[,,(1)]=cpost
    ...
    for (i in 2:nimp) {
      ...
      imp[(i*nrow(X)+1):((i+1)*nrow(X)),(ncol(Y)+3):(ncol(Y)+ncol(X)+2)]=X
      imp[(i*nrow(X)+1):((i+1)*nrow(X)), (ncol(Y)+ncol(X)+3)]=c(1:nrow(Y))
      imp[(i*nrow(X)+1):((i+1)*nrow(X)), (ncol(Y)+ncol(X)+4)]=i
      .Call("jomocox", Ysub, submod, order.sub, Y, Yimp, Yimp2, Y.cat, X, betaY.start,bYpost,betait,bpost,covit,opost, nbetween, l1cov.prior,Y.numcat,ncolYcon,out.iter, PACKAGE="jomo")
      betapost[,,(i-1)]=bpost
      ...
      if (!is.null(Y.con)) {
        imp[(i*nrow(X)+1):((i+1)*nrow(X)),3:(2+ncol(Y.con))]=Yimp2[,1:ncol(Y.con)]
      }
      if (isnullcat==0) {
        imp[(i*nrow(X)+1):((i+1)*nrow(X)),(ncolYcon+3):(2+ncol(Y))]=Y.cat
      }
