jomo1cat <-
  function(Y_cat, Y_numcat, X=matrix(1,nrow(Y_cat),1), betap=matrix(0,ncol(X),((sum(Y_numcat)-length(Y_numcat)))), covp=diag(1,ncol(betap)), Sp=diag(1,ncol(betap)), nburn=100, nbetween=100, nimp=5, meth="MH", rngflag=0) {
    stopifnot((meth=="MH"|meth=="IW"), nrow(betap)==ncol(X), ncol(betap)==((sum(Y_numcat)-length(Y_numcat))),nrow(covp)==ncol(covp), nrow(covp)==ncol(betap), nrow(Sp)==ncol(Sp),nrow(Sp)==nrow(covp))
    ...
    if (meth=="MH") {
     .Call("jomo1mix", Y, Yimp, Yimp2, Y_cat, X,betap,bpost,covp,opost, nburn, Sp,Y_numcat, 0, rngflag, PACKAGE = "jomo")
    }
    if (meth=="IW") {
      .Call("jomo1mix2", Y, Yimp, Yimp2, Y_cat, X,betap,bpost,covp,opost, nburn, Sp,Y_numcat, 0,rngflag, PACKAGE = "jomo")
    }
    #betapost[,,1]=bpost
    #omegapost[,,1]=opost
    bpost<-matrix(0,nrow(betap),ncol(betap))
    ...
    for (i in 2:nimp) {
      ...
      if (meth=="MH") {
        .Call("jomo1mix", Y, Yimp, Yimp2, Y_cat, X,betap,bpost,covp, opost, nbetween, Sp, Y_numcat, 0,rngflag, PACKAGE = "jomo") 
      }
      if (meth=="IW") {
        .Call("jomo1mix2", Y, Yimp, Yimp2, Y_cat, X,betap,bpost,covp, opost, nbetween, Sp, Y_numcat, 0, rngflag, PACKAGE = "jomo") 
      }
      betapost[,,(i-1)]=bpost
      omegapost[,,(i-1)]=opost
      bpost<-matrix(0,nrow(betap),ncol(betap))
