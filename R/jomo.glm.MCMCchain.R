jomo.glm.MCMCchain <-
  function(formula, data, start.imp=NULL, start.imp.sub=NULL, beta.start=NULL, l1cov.start=NULL, l1cov.prior=NULL, betaY.start=NULL, varY.start=NULL, nburn=1000, output=1, out.iter=10, family="binomial") {
    ...
    if (family=="gaussian") {
      jomo.lm.MCMCchain(formula, data,  start.imp=start.imp, start.imp.sub=start.imp.sub, beta.start=beta.start, l1cov.start=l1cov.start, l1cov.prior=l1cov.prior, betaY.start=betaY.start, varY.start=varY.start, nburn=nburn, output=output, out.iter=out.iter)
    }
    if (family=="binomial") {
      ...
      Ysubcat<-c(Ysub)
      .Call("MCMCjomoglm", Ysub, Ysubimp, Ysubcat, submod, order.sub, Y, Yimp, Yimp2, Y.cat, X, betaY.start,betaYpost, betait,betapost, varY.start,varYpost,covit,omegapost, nburn, varY.prior, l1cov.prior,Y.numcat,ncolYcon,out.iter, PACKAGE = "jomo")
