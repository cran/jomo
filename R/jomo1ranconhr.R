jomo1ranconhr <-
  function(Y, X=matrix(1,nrow(Y),1), Z=matrix(1,nrow(Y),1), clus, betap=matrix(0,ncol(X),ncol(Y)), up=matrix(0,length(unique(clus)),ncol(Z)*ncol(Y)), covp=matrix(diag(1,ncol(Y)),length(unique(clus))*ncol(Y),ncol(Y),2), covu=diag(1,ncol(Y)*ncol(Z)), Sp=diag(1,ncol(Y)), Sup=diag(1,ncol(Y)*ncol(Z)), nburn=100, nbetween=100, nimp=5, a=ncol(Y),meth="random", rngflag=0) {
    ...
    if (meth=="fixed") {
      .Call("jomo1ranconhf", Y, Yimp, Yimp2, X, Z, clus, betap, up, bpost, upost, covp,opost, covu,cpost, nburn, Sp, Sup,rngflag=0, PACKAGE = "jomo") 
    }
    if (meth=="random") {
      .Call("jomo1ranconhr", Y, Yimp, Yimp2, X, Z, clus, betap, up, bpost, upost, covp,opost, covu,cpost, nburn, Sp, Sup, a,rngflag=0, PACKAGE = "jomo") 
    }
    #betapost[,,1]=bpost
    #upostall[,,1]=upost
    #omegapost[,,(1)]=opost
    ...
    for (i in 2:nimp) {
      ...
      if (meth=="fixed") {
        .Call("jomo1ranconhf", Y, Yimp, Yimp2, X, Z, clus, betap, up, bpost, upost, covp, opost, covu, cpost, nbetween, Sp, Sup,rngflag=0, PACKAGE = "jomo") 
      }
      if (meth=="random") {
        .Call("jomo1ranconhr", Y, Yimp, Yimp2, X, Z, clus, betap, up, bpost, upost, covp, opost, covu, cpost, nbetween, Sp, Sup, a,rngflag=0, PACKAGE = "jomo")   
      }
      betapost[,,(i-1)]=bpost
      upostall[,,(i-1)]=upost
      omegapost[,,(i-1)]=opost
