jomo1rancat <-
  function(Y_cat, Y_numcat, X=matrix(1,nrow(Y_cat),1), Z=matrix(1,nrow(Y_cat),1), clus, betap=matrix(0,ncol(X),((sum(Y_numcat)-length(Y_numcat)))), up=matrix(0,length(unique(clus)),ncol(Z)*((sum(Y_numcat)-length(Y_numcat)))), covp=diag(1,ncol(betap)), covu=diag(1,ncol(up)), Sp=diag(1,ncol(covp)), Sup=diag(1,ncol(covu)), nburn=100, nbetween=100, nimp=5,meth="MH",rngflag=0) {
    stopifnot((meth=="MH"|meth=="IW"), nrow(betap)==ncol(X), ncol(betap)==((sum(Y_numcat)-length(Y_numcat))),nrow(covp)==ncol(covp), nrow(covp)==ncol(betap), nrow(Sp)==ncol(Sp),nrow(Sp)==nrow(covp),nrow(Z)==nrow(Y_cat), ncol(covu)==ncol(up), ncol(up)==ncol(Z)*((sum(Y_numcat)-length(Y_numcat))))
    ...
    if (meth=="MH") {
      .Call("jomo1ranmix", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betap,up,bpost,upost,covp,opost, covu, cpost, nburn, Sp,Sup,Y_numcat, 0,rngflag, PACKAGE = "jomo")
    }
    if (meth=="IW") {
      .Call("jomo1ranmix2", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betap,up,bpost,upost,covp,opost, covu, cpost, nburn, Sp,Sup,Y_numcat, 0,rngflag, PACKAGE = "jomo")
    }
    #betapost[,,1]=bpost
    #upostall[,,1]=upost
    #omegapost[,,(1)]=opost
    ...
    for (i in 2:nimp) {
      ...
      if (meth=="MH") {
        .Call("jomo1ranmix", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betap,up,bpost,upost,covp,opost, covu, cpost, nbetween, Sp,Sup,Y_numcat, 0, rngflag, PACKAGE = "jomo")
      }
      if (meth=="IW") {
        .Call("jomo1ranmix2", Y, Yimp, Yimp2, Y_cat, X, Z, clus,betap,up,bpost,upost,covp,opost, covu, cpost, nbetween, Sp,Sup,Y_numcat, 0,rngflag, PACKAGE = "jomo")
      }
      betapost[,,(i-1)]=bpost
      upostall[,,(i-1)]=upost
      omegapost[,,(i-1)]=opost
