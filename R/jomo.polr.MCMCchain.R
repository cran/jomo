jomo.polr.MCMCchain <-
  function(formula, data, beta.start=NULL, l1cov.start=NULL, l1cov.prior=NULL, betaY.start=NULL, nburn=1000, start.imp=NULL, start.imp.sub=NULL, output=1, out.iter=10) {
    ...
    fit.cr<-polr(formula,data=data, Hess=T, na.action = na.omit, method = "probit")
    colnamysub<-all.vars(formula[[2]])
    Ysub<-get(colnamysub,pos=data)
    stopifnot(is.factor(Ysub))
    if (is.null(betaY.start)) betaY.start<-coef(summary(fit.cr))[,1]
    Ysub.ncat<-nlevels(Ysub)
    varY.start<-varY.prior<-1
    ...
    varYpost<-array(0, dim=c(1,1,nburn))
    ...
    .Call("jomooprobitC", Ysub, Ysubimp, Ysubcat, submod, order.sub, Y, Yimp, Yimp2, Y.cat.tot, X, betaY.start, betaYpost, betait,betapost, varY.start, varYpost, covit,omegapost, nburn, varY.prior, l1cov.prior,Y.numcat.tot, Ysub.ncat, ncolYcon,out.iter, 1, PACKAGE = "jomo")
    ...
    varYpostmean<-apply(varYpost, c(1,2), mean)
